...
def inicializar_album_completo(user_id):
    """Inicializar álbum completo para un usuario"""
    conn = get_connection()
    cursor = conn.cursor()
...
def update_cromo(user_id, id_cromo, cantidad, nombre=""):
    """Actualizar cantidad de un cromo"""
...
def get_all_cromos(user_id):
    """Obtener todos los cromos de un usuario"""
    inicializar_album_completo(user_id)
    conn = get_connection()
    cursor = conn.cursor()
